...
def load_logs():
    ...
    if last_weekly_reset_str:
        try:
            ...
            if last_weekly_reset < last_friday:
                ...
                print(f"Reset study_log, last_weekly_reset: {last_weekly_reset}")
            else:
                study_log.update(data.get('weekly', {}))
                print(f"Loaded study_log: {study_log}")
        except ValueError:
            study_log = {subject: 0 for subject in SUBJECTS}
            last_weekly_reset = last_friday
            print(f"Invalid last_weekly_reset, reset study_log, last_weekly_reset: {last_weekly_reset}")
    else:
        study_log = {subject: 0 for subject in SUBJECTS}
        last_weekly_reset = last_friday
    ...
    if last_monthly_reset_local != current_month:
        ...
        print(f"Reset monthly_log, last_monthly_reset: {last_monthly_reset}")
    else:
        monthly_log.update(data.get('monthly', {}))
        last_monthly_reset = last_monthly_reset_local
        print(f"Loaded monthly_log: {monthly_log}")
