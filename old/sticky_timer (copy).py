...
last_monthly_reset = datetime.now().strftime('%Y-%m')  # Tracks the last monthly reset month
...
def load_logs():
    ...
    # Monthly reset (fixed)
    last_monthly_reset_local = data.get('last_monthly_reset', current_month)  # Default to current month if missing
    if last_monthly_reset_local != current_month:
        ...
        last_monthly_reset = last_monthly_reset_local  # Retain loaded value if no reset
...
def reminder():
    ...
    if not timer_started and not concentration_mode_active:
        ...
        # Set reminder popup size and position
        ...
        # Reschedule the reminder for 5 seconds later
